...
<html lang="UTF-8">
  <head>
    ...
    <!-- 
    	<base >   以后所有的相对路径都有了基准路径？
    	使用绝对路径
    -->
    <!-- 相对路径容易出问题，我们都推荐写 加上项目名的以/开始的 绝对路径 -->
	<%--
		引入公共资源：(推荐放在WEB-INF下供公共引入而不是让其他人直接访问)
		 静态包含： <%@include file="" %>   80%
		 	结果：  A.jsp 要 包含  B.jsp：
		 		所有的jsp都会编译成.class，然后运行；
		 	效果：将被包含页面的内容CV进A.jsp里面，然后只编译和运行A.jsp：（产生一个.class文件）
		 动态包含：<jsp:include page=""></jsp:include>  20%
		 	效果：将A.jsp页面运行的结果 和 B.jsp页面运行的结果进行合并
		 		编译了两个jsp，生成两个.class
		区别：被包含的页面需要运行一下；
			如果被包含页面的内容经常发生变化或者是一些动态代码的，需要实时运行出效果的，我们就用动态包含
	 --%> 
...
<div class="container">
	...
	<!-- 
	              服务器解析的/开始的路径，从当前项目开始算，不用加项目名；
		  浏览器解析的/开始的路径，从服务器下开始算（localhost:8080），必须加上项目名；
		 只要不是浏览器解析的就是服务器解析？
		 	浏览器解析的？只要是HTML标签的都是浏览器解析,response.sendRedirect("/aa/aa")(浏览器解析重定向)
