...
public interface TProjectMapper {
	...
	List<TProject> selectHotsProject(@Param("hots")String hots,@Param("tid") Integer typeId);

	List<TProject> selectProjects(@Param("tid")Integer typeId, 
			@Param("pstatus")String projectStatus, 
			@Param("ob")String orderBy);
}